...
import { TokenRepositoryImpl } from '@core/infrastructure/repositories/token-repository';
import {
  Body,
  Controller,
  Delete,
...
import {
  ...
  TokensUpdateDto,
  TokensViewDto,
} from '../dtos/tokens.crud.controllers.dto';
...
export class TokensCrudController {
  ...
  async findAll(@Body() body: TokensGetDto) {
    const tokensGetUsecase = new TokensGetUsecase({
      tokensRepository: this.repositoryImpl,
    });
